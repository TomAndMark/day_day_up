...
int rbtree<T>::erase(const T& item)	//1:删除成功，0 :删除失败
{
	...
	if (x != NIL)
	{
		...
		else			//当左右节点都不为空时 找后继节点(转化为有一个孩子为空的节点)
...
{
	return erase(*pos);//不能用erase(pos.nodePtr->value);//nodePtr私有的
}
...
template <class T>
rbtree<T>::rbtree(const rbtree<T>& obj)//为何在这里：treeSize（obj.treeSize）不管用呀？？
{
	makeEmptyTree();
