...
{
	String s1;//需要给s1分配空间，不然后面的语句不会输出
	// cout <<"s1[0]: "<<s1[2] << endl;
   String s2(5,'c');
String s3("attacccca");
	//String s3("ccccc");
	String s4("abcd efg higk",6);
    String s5("abcd efg higk",5,15);
	char a[20] = "cacdefghijllmfdhtu";
	char *b = a+10;//指向l  答案是llmfd
	char *c = a+15;//指向h
	//String s6(b,c);
	cout<<"s1:"<< s1.size()<<endl;//输入输出的时候符号不要反了
	cout<<"s2:"<< s2.capacity()<<endl;//ccccc
	
	cout << s3.capacity() << endl;
	s3 = s2;
	cout << s3.capacity() << endl;
	cout << "s1:" << s3.empty() << endl;
	...
	cout<<"s3:"<< s3 << s3.capacity()<<endl;//abcd
	cout<<"s3:"<< s3.at(2) << endl;
	// cout<<"s3:"<< (s3.c_str()="123") << endl;
	char aa[10] = "\0";
	cout <<"s3:" << (s3.copy(aa, 20, 0)) << endl;
	cout << "aa:"<< aa << endl;

	cout<<"s4:"<< s4.capacity()<<endl;
	cout<< "s5:" << s5 << endl;
	cout << "s5size:" << s5.size() <<endl;
	//cout<<s6<<endl;
	// cout<<s2+s3<<endl;
	// cout<<(s2=s3)<<endl;
	// cout<<(s2+=s3)<<endl;//自动扩充了空间
	//cout<<(s2==s3)<<endl;
	cout<<s3[4]<<endl;
	cout<< "append:"<< s2.append(b,c)<<endl;
	cout << "s2size:" << s2.size() <<endl;
	//cout<<s2.assign(s3,1,2)<<endl;
    try
	{
		char ch = s3.at(1);
		cout<<ch<<endl;
	...
	//cout<<s2.compare(1,2,s3,1,2)<<endl;
	//cout<<s2.copy(a,3,1)<<endl;
	//cout<<a<<endl;
	//cout<<s2.end()<<endl;
	cout<< "erase:" <<s3.erase(6)<<endl;
	cout<< "erase:" <<s3.size()<<endl;

	cout<<s3.find("cc",3)<<endl;//****注意调用是是'c'  不是“c”
	cout<<s3.length()<<endl;
	cout<<s3.max_size()<<endl;//************8错误
	// s3.swap(s2);
	cout<<"s2:"<<s2<<endl;
	cout<<"s3:"<<s3<<endl;
	cout<<s3.replace(1,4,s2, 6,3)<<endl;
	cout<<s3<<endl;
	// cout<<s3.insert(1,s2)<<endl;
	// cout<<s3<<endl;

	// cout<<s3.replace(1,3,5,'b')<<endl;
	// cout<<s3<<endl;

 /*   int loc;
    String s = "My cat's breath smells like cat food.";

    loc = s.rfind( "breath", 7,1);
    cout << "The word breath is at index " << loc << endl;*/
    //cout<<s3.substr(1,2)<<endl;
    
    //cout<<s3.insert(1,5,'2')<<endl;
    //cout<<s3<<endl;
    cout<<s3.find_last_of('c',2)<<endl;

    String s7 = "aaa";  //编译器优化调用的是String s7("aaa");
    cout << "s7:" << s7 <<endl;
    ...
    loc = s.rfind( "breath", 8, 9);
    cout << "The word breath is at index " << loc << endl;

    loc = s.rfind( "breath", 20, 21);
    cout << "The word breath is at index " << loc << endl;
